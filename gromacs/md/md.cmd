...
REM md, for performing a molecular dynamics simulation using GROMACS.
...
REM Create run file.
grompp -f %INPUT_RUNFILE% -po %TEMP_MDP% -c %INPUT_CONFORMATION% -p %INPUT_TOPOLOGY% -o %TEMP_TPR% > nul

REM Execute run file.
mdrun -s %TEMP_TPR% -x %OUTPUT_TRAJECTORY% -cpo %TEMP_CPT% -c %TEMP_GRO% -e %OUTPUT_ENERGY% -g %TEMP_LOG% > nul
